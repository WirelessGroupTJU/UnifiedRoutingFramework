...
static bool mac_address_cmp(unsigned char *mac_address1, unsigned char* mac_address2){
    for(int i=0;i<ETH_ALEN;i++){
        if(mac_address1[i] != mac_address2[i])
            return false;
    }
...
int ODS_get_route_request(route_request* now_request){
    ...
    now_request->input_ifindex = -1;

    //get interface index from mac address
    if(now_info->packet_source_type == PACKET_FROM_OTHER){
        //get inport mac address
        unsigned char mac_addr[ETH_ALEN];
        memcpy(mac_addr, now_info->mac_dst_addr, ETH_ALEN);
        ...
        for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
            if(!strcmp(ifa->ifa_name, TUN_DEV_NAME)){
                continue;
            }

            unsigned char interface_adress[ETH_ALEN];
            int ret = get_mac(ifa->ifa_name, interface_adress);
            if(ret !=0 ){
                printf("in dev not find\n");
                return -1;
            }
            // printf("%02x:%02x:%02x:%02x:%02x:%02x\n", interface_adress[0], interface_adress[1], interface_adress[2]
            //                                 , interface_adress[3], interface_adress[4], interface_adress[5]);

            if(mac_address_cmp(mac_addr, interface_adress)){
                int in_inerfacae_index = if_nametoindex((const char*)mac_addr);
                now_request->input_ifindex = in_inerfacae_index;
                printf("[ODS_get_route_request] find indev, name is %s\n", ifa->ifa_name);
                break;
            }
        }
